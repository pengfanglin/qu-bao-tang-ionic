...
<script type="text/ecmascript-6">
  export default {
    ...
    mounted() {
      ...
      this.$store.commit('leftMenuList', this.systemModules);
      if (!this.isEmpty(this.systemModules) && !this.isEmpty(this.systemModules[0].systemModules)) {
        let ids = this.defaultModuleId.split(',');
        this.$router.push(this.systemModules[ids[0]].systemModules[ids[1]].moduleUrl);
      }
    },
    methods: {
      open(index) {
