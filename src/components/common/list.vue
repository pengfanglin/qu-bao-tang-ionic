<template>
  <div class="list_root">
    ...
    <el-table
      ...
      style="width: 100%"
    >
      <el-table-column v-if="!isEmpty(checkKey)" type="selection" width="40"></el-table-column>
      <el-table-column
        align="center"
        v-for="(base,column) in baseData"
        :width="base.width ? base.width : ''"
        ...
        <template slot-scope="scope">
          <div v-if="base.type==='img'" class="box_center">
            <img
              ...
              @click="changeBig(scope.$index,$event)"
              @error="imgError"
            />
          </div>
          <div v-else-if="base.type==='operation'" class="box_center">
            <el-button
              ...
              @click="operationClick(index,scope.$index,base.data)"
            >{{operation.name}}</el-button>
          </div>
          <div v-else-if="base.type === 'tooltip'" class="box_center tooltip">
            <span v-if="scope.row[base.key].split('').length < 60">
              {{
              scope.row[base.key].split('').filter((e, index) => index < 60).join('')
              }}
            </span>

            <el-popover
              v-if="scope.row[base.key].split('').length > 60"
              placement="top-start"
              width="300"
              trigger="hover"
              :content="scope.row[base.key]"
            >
              <span slot="reference">查看</span>
            </el-popover>
          </div>
          <div v-else-if="base.type === 'workLocation'" class="workLocation_box_center">
            <div v-for="(item, index) in scope.row[base.key]">
    ...
    <img
      v-if="showImgRowId!==-1"
      :src="src.indexOf('http')>-1?src:(homeUrl+src)"
      @click="changeSmall"
      @error="imgError"
      class="img_click"
    />
...
export default {
  ...
  methods: {
    ...
    uploadImage(rowId, filed, path, e) {
      ...
      if (
        e.target.value.indexOf(".jpg") > -1 ||
        e.target.value.indexOf(".jpeg") > -1 ||
        e.target.value.indexOf(".png") > -1 ||
        e.target.value.indexOf(".gif") > -1
      ) {
        this.photoCompress(
          e.target.files[0],
          {
...
.box_center > .examine.el-tooltip.item {
